...
void binary_tree_levelorder(const binary_tree_t *tree, void (*func)(int))
{
	...
	for (mlevel = 1; mlevel <= smaxlevel; mlevel++)
		sbtlo_helper(tree, func, mlevel);
}

/**
 * sbtlo_helper - aaaaaaaaaaaaaaaaaaaaaaa
 * @tree: fffffffffffffffffffffff
 * @func: hhhhhhhhhhhhhhhhhhhhhhh
 * @mlevel: jjjjjjjjjjjjjjjjjjjjjjjjjjj
 */
void sbtlo_helper(const binary_tree_t *tree, void (*func)(int), size_t mlevel)
{
	if (mlevel == 1)
		func(tree->n);
	else
	{
		sbtlo_helper(tree->left, func, mlevel - 1);
		sbtlo_helper(tree->right, func, mlevel - 1);
	}
}
